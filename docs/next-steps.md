...
## Closed — theme-index experiment: null result (2026-08-04)

Ran end to end. The isolated theme vector discriminates genuine resonance from
coincidence no better than the raw book vector — gaps of 0.0057 and 0.0035
against within-group spreads two orders of magnitude larger. Numbers and the
caveat on label provenance: `docs/experiments/theme-index/RESULT.md`; the
design consequence is ADR-010's second amendment.

What it means in practice: **three automatic signals have now failed the same
way, so stop buying the next one on spec.** The LLM relevance judge is the
gate without an asterisk, and the cost motive behind wanting a pre-LLM signal
was met the same day by giving DIGEST its own model tier.

That leaves **P2 (source breadth) as the only remaining lever on the base rate
of genuine resonance** — 9 genuine in 30 is what world news against a
1,400-book general shelf produces, and no amount of signal engineering changes
the numerator.
